...
class SpotifyWidget extends MatrixWidget<SpotifyLayer> {
  ...
  void _renderArtAndText(SpotifyLayer layer, PixelBuffer buffer, int elapsedMs, SpotifyTrack track) {
  ...
  void _renderTextOnly(SpotifyLayer layer, PixelBuffer buffer, int elapsedMs, SpotifyTrack track) {
    final font = LedFontLibrary.get(layer.fontId);
    final int fontH = font.charHeight;
  ...
  void _renderArtOnly(SpotifyLayer layer, PixelBuffer buffer, SpotifyTrack track) {
  ...
  void _drawText(
    ...
    int elapsedMs,
    double baseOpacity,
    AnimationEffect effect,   // ← AnimationEffect directly, no mapping needed
    int speedMs,
  ) {
    switch (effect) {
      ...
      case AnimationEffect.scrollRight:
        _scrollText(font, buffer, text, color, startX, y, maxW,
            elapsedMs, baseOpacity, speedMs: speedMs);
      ...
      case AnimationEffect.pulse:
        ...
        final double op = ((1 - math.cos(phase * 2 * math.pi)) / 2) * baseOpacity;
        _scrollText(font, buffer, text, color, startX, y, maxW,
            elapsedMs, op.clamp(0, 1), speedMs: speedMs);

      case AnimationEffect.fade:
        final int t = elapsedMs % 3000;
        final double op = t < 1500 ? baseOpacity : 0.0;
        _scrollText(font, buffer, text, color, startX, y, maxW,
            elapsedMs, op, speedMs: speedMs);

      case AnimationEffect.burst:
        final int t = elapsedMs % 1500;
        final double op = t < 400
            ? (1.0 - t / 400) * baseOpacity
            : 0.12 * baseOpacity;
        _scrollText(font, buffer, text, color, startX, y, maxW,
            elapsedMs, op.clamp(0, 1), speedMs: speedMs);
    }
  ...
  void _scrollText(
    LedFont font, PixelBuffer buffer, String text, Color color,
    int startX, int y, int maxW, int elapsedMs, double opacity, {
    int speedMs = 60,
  }) {
  ...
  // ── Album art + progress bar (unchanged) ──────────────────────────────────
  ...
  void _blitArt(PixelBuffer dst, SpotifyTrack track,
    ...
    switch (mode) {
      ...
      case ArtLayoutMode.letterbox:
        ...
        if (sa > da) { dstHeight = (w / sa).round(); dstY = y + ((h - dstHeight) ~/ 2); }
        else         { dstWidth  = (h * sa).round(); dstX = x + ((w - dstWidth)  ~/ 2); }
      case ArtLayoutMode.fill:
        final double sa = srcW / srcH, da = w / h;
        if (sa > da) { srcWidth  = (srcH * da).round(); srcX = (srcW - srcWidth)  ~/ 2; }
        else         { srcHeight = (srcW / da).round(); srcY = (srcH - srcHeight) ~/ 2; }
    }
